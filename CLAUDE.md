...
5. ~~`migrate.js` MIGRATIONS list missing 8 files~~ **RESOLVED**: boot + CLI now share `db/migrations.js` (all 15 files, dependency-ordered) so the two lists can't drift. Add a migration by dropping the `.sql` in `db/` and appending its name there (must be idempotent).
