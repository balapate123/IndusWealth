...
const HomeScreen = ({ navigation }) => {
    ...
    return (
        ...
        >
            ...
            {/* Watchdog used to have a summary card here. Removed by user
                decision: the feature is not yet pulling its weight, and a card
                on the busiest screen is a claim that it is. It stays reachable
                from Profile -> Quick access, where an unfinished feature can
                sit without competing for attention it has not earned.

                The summary fetch went with it. Leaving the request in while the
                card is gone would cost every Home load a round trip for a value
                nothing renders -- the kind of thing that survives for years
                because it is invisible. */}

            {/* Goals   <Ionicons name="chevron-forward" size={18} color={theme.TEXT_MUTED} />
                    </View>
                </Card>
            </View>
